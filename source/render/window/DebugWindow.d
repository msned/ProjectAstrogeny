...
class DebugWindow : WindowObject {
	...
	protected override void loadRenderObjects() {
		...
		regions ~= left;
		ResponsiveRegion topRight = new VerticalGridRegion(new AnchorRegion(left, Side.left), new AnchorPercentage(1f, Side.top), new AnchorPercentage(1f, Side.right), new DragPercentage(.75f, Side.bottom, new RegionBoarder(false, this)));
		regions ~= topRight;
